...
mir_cutoff_SQ_mol = read.csv(file="mamed-separator-miRNAs-for-SQvsNorm-rpkm.csv", header=T, row.names = 1,stringsAsFactors = F)
mir_count_SQ_mat = my_SQ_mat[as.character(mir_cutoff_SQ_mol[1,]),]
mir_count_SQ_mat[7,] = c(rep(1,times=56),rep(0, times=22))
rownames(mir_count_SQ_mat)[7] = "condition" 
tr_mir_SQ_count = as.data.frame(t(mir_count_SQ_mat))
my_v_SQ_RPM = cbind(v_SQ_mol_RPM,v_SQ_n_mol_RPM)
mir_SQ_v_rpm = v_SQ_RPM[as.character(mir_cutoff_SQ_mol[1,]),]
mir_SQ_v_rpm[7,] = c(rep(1,times=60),rep(0, times=23)) 
rownames(mir_SQ_v_rpm)[7] = "condition"
colnames(mir_SQ_v_rpm)= make.names(names = rep(x="x",times=83),unique = T)
tr_SQ_v_rpm = as.data.frame(t(mir_SQ_v_rpm))
