...
setwd("/media/egedede/New Volume/masterGIREMI/bcftools_out")
fnames = list.files(path = "/media/egedede/New Volume/masterGIREMI/bcftools_out", full.names=T)
print(fnames)
for(i in 1:length(fnames)){
...
system("sudo rm  /media/egedede/\"New Volume\"/masterGIREMI/bcftools_out/* ",wait= T, intern = T)
system("sudo rm /media/egedede/\"New Volume\"/masterGIREMI/pileup/* ",wait= T, intern = T)
