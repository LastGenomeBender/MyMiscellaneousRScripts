...
ad2 = rep(x= 0, times =6)
response = c(ad1, ad2)
lasso_data2 = cbind(x2,response) 
lasso_data2 = as.data.frame(lasso_data2)
colnames(lasso_data2)[ncol(lasso_data2)] = "resp"
xy <- model.matrix(resp~.,lasso_data2)
#conT2ert class to numerical T2ariable

cv.out <- cv.glmnet(xy,as.numeric(t(lasso_data2[ncol(lasso_data2)])),alpha=1,family="binomial",type.measure = "mse" )
#plot result
plot(cv.out)
...
######### ROC
t3_brca =read.csv("T2_BRCA_TCGA.csv", row.names=1)
