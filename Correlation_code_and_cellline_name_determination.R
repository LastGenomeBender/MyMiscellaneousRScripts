...
load("/media/ahadli/Data/Farid/AOG_lab/Rna Editing GIREMI/SNV_AG_analysis/A-to-I GIREMI Colon CCLE/all_lists.RData")
...
for(i in diff_compounds){
  ...
  edit_temp = edit_temp[apply(edit_temp, 1, FUN=function(x) sum((sum(x>0))))>10,] 
  sp_pval = c()
  pe_pval = c()
  sp_r = c()
  pe_r =c()
...
setwd("/media/ahadli/Data/Farid/AOG_lab/Rna Editing GIREMI/SNV_AG_analysis/A-to-I GIREMI Colon CCLE/Compouds_correlations")
...
nope=read.csv("/media/ahadli/Data/Farid/AOG_lab/Rna Editing GIREMI/SNV_AG_analysis/A-to-I GIREMI Colon CCLE/Compouds_correlations/Paclitaxel_correlation.csv", row.names = 1, header = T, stringsAsFactors = F)
head(nope)
colnames(aaaa)
